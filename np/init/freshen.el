(defun np/delete-out-of-date-bytecode (directory)
  ...
  (dolist (path-and-attributes (directory-files-and-attributes directory t))
    (let ((path       (car path-and-attributes))
      ...
      (cond ((string-prefix-p "." (file-name-nondirectory path))
            ...
            ((car attributes)
             (np/delete-out-of-date-bytecode path))
...
(np/delete-out-of-date-bytecode
 (expand-file-name "np" user-emacs-directory))
