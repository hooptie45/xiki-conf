...
;;;###autoload
(defun np/rails-console ()
  "Invoke inf-ruby with Rails environment loaded."
  (interactive)
  (let ((config (np/find-in-path "config/environment.rb")))
    (if config
...
;;;###autoload
(defun np/rails-dbconsole ()
  "Open a SQL shell using the settings from config/database.yml."
  (interactive)
  (let ((config (np/find-in-path "config/database.yml")))
    (if config
        (let* ((env     (np/read-rails-database-config config))
               (adapter (car env))
